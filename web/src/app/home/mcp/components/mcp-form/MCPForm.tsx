...
const MCPForm = forwardRef<MCPFormHandle, MCPFormProps>(function MCPForm(
  ...
  const loadServerForEditRef = useRef(loadServerForEdit);
  const testMcpRef = useRef(testMcp);

  const watchMode = form.watch('mode');
  const runtimeStatus = runtimeInfo?.status;
  loadServerForEditRef.current = loadServerForEdit;
  testMcpRef.current = testMcp;
  ...
  useEffect(() => {
    ...
    if (isEditMode && initServerName) {
      loadServerForEditRef.current(initServerName).finally(() => {
  ...
  }, [form, initServerName, isEditMode]);
  ...
  useEffect(() => {
    if (
      !isEditMode ||
      !initServerName ||
      runtimeStatus !== MCPSessionStatus.CONNECTING
    ) {
      if (pollingIntervalRef.current) {
        clearInterval(pollingIntervalRef.current);
        pollingIntervalRef.current = null;
    ...
    if (!pollingIntervalRef.current) {
      pollingIntervalRef.current = setInterval(() => {
        void loadServerForEditRef.current(initServerName);
      }, 3000);
    }
  ...
  }, [initServerName, isEditMode, runtimeStatus]);
