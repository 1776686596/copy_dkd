...
import {
  ...
  ServiceDeskBotConfigUpdatePayload,
  ServiceDeskAssistDraft,
  ServiceDeskQuickReply,
...
export class BackendClient extends BaseHttpClient {
  ...
  public getServiceDeskSessions(
    params: ServiceDeskSessionQuery,
    ...
    if (params.queueStatus)
      queryParams.append('queueStatus', params.queueStatus);
  ...
  public getServiceDeskSessionDetail(
    sessionId: string,
  ): Promise<ServiceDeskSessionDetail> {
  ...
  public getServiceDeskBotConfigs(): Promise<{
    ...
    return this.get('/api/v1/service-desk/bot-configs');
  }

  public updateServiceDeskBotConfig(
    botUuid: string,
    payload: ServiceDeskBotConfigUpdatePayload,
  ): Promise<object> {
    return this.put(`/api/v1/service-desk/bot-configs/${botUuid}`, payload);
  }

  public getServiceDeskMaterials(botUuid: string): Promise<{
    items: ServiceDeskMaterial[];
  }> {
    return this.get(`/api/v1/service-desk/bots/${botUuid}/materials`);
  }

  public getServiceDeskQuickReplies(botUuid: string): Promise<{
    items: ServiceDeskQuickReply[];
  }> {
    return this.get(`/api/v1/service-desk/bots/${botUuid}/quick-replies`);
  }

  public createServiceDeskMaterial(
    botUuid: string,
    payload: Omit<
      ServiceDeskMaterial,
      'uuid' | 'bot_uuid' | 'created_at' | 'updated_at'
    >,
  ): Promise<{ uuid: string }> {
    return this.post(`/api/v1/service-desk/bots/${botUuid}/materials`, payload);
  }

  public claimServiceDeskSession(sessionId: string): Promise<object> {
    return this.post(`/api/v1/service-desk/sessions/${sessionId}/claim`);
  }

  public setServiceDeskSessionMode(
    sessionId: string,
    mode: 'ai_hosted' | 'manual' | 'ai_assist',
  ): Promise<object> {
    return this.post(`/api/v1/service-desk/sessions/${sessionId}/mode`, {
      mode,
    });
  }

  public generateServiceDeskAssistDraft(sessionId: string): Promise<{
    draft: ServiceDeskAssistDraft;
  }> {
    return this.post(`/api/v1/service-desk/sessions/${sessionId}/assist-draft`);
  }

  public releaseServiceDeskSession(sessionId: string): Promise<object> {
    return this.post(`/api/v1/service-desk/sessions/${sessionId}/release`);
  }

  public returnServiceDeskSessionToAi(sessionId: string): Promise<object> {
    return this.post(`/api/v1/service-desk/sessions/${sessionId}/return-ai`);
  }

  public replyServiceDeskSession(
    sessionId: string,
    replyText: string,
  ): Promise<object> {
