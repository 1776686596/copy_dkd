...
- AI profile configuration continues to reuse the existing Pipeline page.
...
- Added regression coverage:
  ...
  - verifies `manual -> silent -> ai_hosted` helper flow
  - verifies handoff keywords move sessions into `pending_manual`
- Manual workbench flow to verify:
  1. Create a `wecomcs` bot and bind a pipeline.
  2. Configure service-desk keywords and materials from `/home/service-desk`.
  3. Send a normal FAQ and confirm the session stays in `ai_hosted`.
  4. Send `下载` and confirm the bot returns the structured material reply.
  5. Send a configured handoff keyword and confirm the queue becomes `pending_manual`.
  6. Claim the session and send a manual reply from the workbench.
  7. Wait beyond `manual_timeout_seconds` and confirm the session enters `silent`.
  8. Send the next customer message and confirm the session reopens to `ai_hosted`.
