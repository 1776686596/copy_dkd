...
class RuntimeBot:
    ...
    async def _handle_service_desk_before_pipeline(
        self,
        event: platform_events.MessageEvent,
        adapter: abstract_platform_adapter.AbstractMessagePlatformAdapter,
        pipeline_uuid: str | None = None,
    ) -> bool:
        if not hasattr(self.ap, 'service_desk_service'):
            return False
    ...
    async def initialize(self):
        async def on_friend_message(
            ...
            if not skip_pipeline:
                ...
                if await self._handle_service_desk_before_pipeline(event, adapter, pipeline_uuid=pipeline_uuid):
                    return
