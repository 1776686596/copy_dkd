...
class WecomWebMessageConverter(abstract_platform_adapter.AbstractMessageConverter):
    @staticmethod
...
class WecomWebEventConverter(abstract_platform_adapter.AbstractEventConverter):
    @staticmethod
    async def yiri2target(event: platform_events.MessageEvent) -> WecomWebMessageEvent:
        return event.source_platform_object

    @staticmethod
    async def target2yiri(event: WecomWebMessageEvent):
        message_chain = await WecomWebMessageConverter.target2yiri(event.content, event.message_id)
...
class WecomWebAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    bot: WecomWebPageClient = pydantic.Field(exclude=True)
    message_converter: WecomWebMessageConverter = WecomWebMessageConverter()
    event_converter: WecomWebEventConverter = WecomWebEventConverter()
    bot_uuid: str | None = None
    listeners: dict = pydantic.Field(default_factory=dict, exclude=True)

    def __init__(self, config: dict, logger: abstract_platform_logger.AbstractEventLogger):
        required_keys = ['account_label', 'workbench_url', 'storage_state_dir']
        missing_keys = [key for key in required_keys if not config.get(key)]
        if missing_keys:
            raise command_errors.ParamNotEnoughError('企微网页托管缺少相关配置项，请查看文档或联系管理员')

        bot = WecomWebPageClient.from_config(config, logger=logger)

        super().__init__(
            config=config,
            logger=logger,
            bot_account_id=config.get('account_label', ''),
            listeners={},
            bot=bot,
        )
        self.listeners = {}
    ...
    async def reply_message(
        ...
        event_converter = getattr(type(self), 'event_converter', WecomWebEventConverter())
        message_converter = getattr(type(self), 'message_converter', WecomWebMessageConverter())
    ...
    def extract_service_desk_context(self, event: WecomWebMessageEvent) -> dict[str, str]:
        return {
            'source_entry_id': event.account_id or '',
    ...
    async def _handle_browser_message(self, payload: dict[str, typing.Any]):
        event = WecomWebMessageEvent(
            ...
            sender_name=payload.get('sender_name') or payload['external_user_id'],
            content=payload.get('content', ''),
            timestamp=int(payload.get('timestamp') or datetime.datetime.now().timestamp()),
        )
        object.__setattr__(self, 'bot_account_id', event.account_id)
        try:
            event_converter = getattr(type(self), 'event_converter', WecomWebEventConverter())
            ...
            await self.logger.error(f'Error in wecomweb callback: {traceback.format_exc()}')
